        ...
        <div className="map-column">
          ...
          <div className="map-frame" ref={mapRef} onContextMenu={(event) => { event.preventDefault(); setSelectedId(null); }}>
            ...
            {selectedDossier && selected && <aside className="country-dossier" aria-label={`Informacje o państwie ${selected.country.name}`}>
              ...
              {dossierTab === "military" && <section className="dossier-regions"><header><b>KONTROLOWANE SEKTORY</b><span>{selectedDossier.regions.length}</span></header><div>{selectedDossier.regions.slice(0, 8).map((region) => <button key={region.id} className={inspectedSectorId === region.id ? "active" : ""} onClick={() => { setInspectedSectorId(region.id); paint(); }}><span>{region.name}</span><b>{formatArea(region.areaKm2)}</b></button>)}</div>{selectedDossier.regions.length > 8 && <small>oraz {selectedDossier.regions.length - 8} kolejnych sektorów</small>}</section>}
              {dossierTab === "logistics" && <section className="logistics-dossier"><div><small>INDEKS LOGISTYCZNY <InfoTip>Ocena sprawności transportu i zaopatrzenia w skali 0–100. Uwzględnia porty, kolej, drogi, połączenia lotnicze i inwestycje.</InfoTip></small><strong>{typeof selectedDossier.countryLogistics === "number" ? Math.round(selectedDossier.countryLogistics) : "—"}</strong><em>/100</em></div><div><span><small>WPŁYW NA GOSPODARKĘ <InfoTip>Słaba logistyka obniża gospodarkę, a bardzo dobra wspiera odbudowę i utrzymanie wojska.</InfoTip></small><b>{selectedDossier.countryLogistics < 30 ? "-0.05/tk" : selectedDossier.countryLogistics > 80 ? "+0.03/tk" : "neutralny"}</b></span><span><small>WPŁYW NA REZERWY <InfoTip>Przy indeksie poniżej 40 ogranicza liczbę dostępnych rezerw.</InfoTip></small><b>{selectedDossier.countryLogistics < 40 ? "-0.02/tk" : "neutralny"}</b></span></div><details><summary>{(selectedDossier.regionLogistics ?? []).length} regionów <InfoTip>Kliknij, aby zobaczyć stan infrastruktury każdego sektora.</InfoTip></summary><div className="logistics-regions">{(selectedDossier.regionLogistics ?? []).slice(0, 12).map((item) => <span key={item.regionId}><small>{strategicRegions[item.regionId]?.name ?? `Region ${item.regionId}`} <InfoTip>Indeks sektora łączy wszystkie widoczne niżej elementy infrastruktury.</InfoTip></small><b>{Math.round(item.logisticsIndex)}</b><small>port {Math.round(item.maritimeAccess)}/100 <InfoTip>Dostęp do portów i żeglugi. Rozbudowa portu działa tylko w sektorze nadmorskim.</InfoTip> · kolej {Math.round(item.railDensity)}/100 <InfoTip>Poziom sieci kolejowej, nie liczba kilometrów. Modernizacja kolei podnosi go w wybranym sektorze, wojna i utrata sektora odbierają jego korzyści.</InfoTip> · drogi {Math.round(item.roadDensity)}/100 <InfoTip>Poziom sieci drogowej, nie liczba kilometrów. Modernizacja dróg podnosi go w wybranym sektorze, a wojna może ograniczyć dostępne zaplecze.</InfoTip> · lotnicze {Math.min(100, item.airportCount * 25)}/100 <InfoTip>Zdolność obsługi lotniczej sektora, a nie deklarowana liczba prawdziwych lotnisk. Rozbudowa lotniska podnosi ją w wybranym sektorze.</InfoTip></small></span>)}</div></details></section>}
              ...
              {dossierTab === "military" && inspectedSectorId !== null && (() => { const sector = strategicRegions[inspectedSectorId]; const ownerId = sector?.ownerId ?? selectedDossier.countryId; const owner = engine?.getCountry(ownerId); const regionLogistics = engine?.getRegionLogistics(inspectedSectorId); const defense = engine?.getStrategicRegionResistance(inspectedSectorId); const terrain = engine?.getStrategicRegionTerrain(inspectedSectorId); return <section className="sector-detail"><header><b>{sector?.name ?? `Sektor ${inspectedSectorId}`}</b><span>{owner?.flag} {owner?.name ?? "—"}</span></header><div><span><small>Obrona <InfoTip>Łączy rozmiar sektora, jego złożoność administracyjną i rzeźbę terenu. Wyższa wartość spowalnia atakującego.</InfoTip></small><b>{defense?.label ?? "—"}</b></span><span><small>Teren <InfoTip>Rzeźba terenu daje niewielką przewagę obrońcy: teren pofałdowany i górzysty jest trudniejszy do szybkiego opanowania.</InfoTip></small><b>{terrain?.label ?? "—"}</b></span><span><small>Logistyka <InfoTip>Określa sprawność dowozu ludzi i zaopatrzenia do sektora.</InfoTip></small><b>{typeof regionLogistics === "number" ? Math.round(regionLogistics) : "—"}</b></span></div><footer><p>{formatArea(sector?.areaKm2 ?? 0)}. {terrain?.description ?? ""} {sector?.provinceNames.length ? sector.provinceNames.join(" · ") : ""}</p></footer></section>; })()}
            </aside>}
        ...
        <aside className="controls">
          ...
          {gameMode === "strategy" ? <div className="strategy-command" aria-live="polite">
            ...
            <div className="strategy-player">
              ...
              {!selectedDossier && typeof playerDossier?.countryLogistics === "number" && <section className="logistics-card"><header><b>LOGISTYKA <InfoTip>Ocena transportu i zaopatrzenia kraju w skali 0–100.</InfoTip></b><span>{Math.round(playerDossier.countryLogistics)}/100</span></header><div className="logistics-rows"><span><small>Indeks <InfoTip>Łączy porty, kolej, drogi i połączenia lotnicze.</InfoTip></small><b>{Math.round(playerDossier.countryLogistics)}</b></span><span><small>Wpływ na gospodarkę <InfoTip>Słaba logistyka obniża gospodarkę, dobra wspiera odbudowę.</InfoTip></small><b>{playerDossier.countryLogistics < 30 ? "-0.05/tk" : playerDossier.countryLogistics > 80 ? "+0.03/tk" : "neutralny"}</b></span><span><small>Wpływ na rezerwy <InfoTip>Przy indeksie poniżej 40 ogranicza liczbę dostępnych rezerw.</InfoTip></small><b>{playerDossier.countryLogistics < 40 ? "-0.02/tk" : "neutralny"}</b></span></div><details><summary>{(playerDossier?.regionLogistics ?? []).length} regionów <InfoTip>Stan infrastruktury poszczególnych sektorów.</InfoTip></summary><div className="logistics-regions">{(playerDossier?.regionLogistics ?? []).slice(0, 12).map((item) => <span key={item.regionId}><small>{strategicRegions[item.regionId]?.name ?? `Region ${item.regionId}`}</small><b>{Math.round(item.logisticsIndex)}</b><small>port {Math.round(item.maritimeAccess)}/100 · kolej {Math.round(item.railDensity)}/100 · drogi {Math.round(item.roadDensity)}/100 · lotnicze {Math.min(100, item.airportCount * 25)}/100 <InfoTip>Wszystkie wartości są poziomami infrastruktury w skali gry, nie kilometrami ani liczbą prawdziwych lotnisk. Polityki infrastrukturalne wzmacniają wybrany sektor.</InfoTip></small></span>)}</div></details></section>}
