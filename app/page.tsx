        ...
        <div className="map-column">
          ...
          <div className="map-frame" ref={mapRef} onContextMenu={(event) => { event.preventDefault(); setSelectedId(null); }}>
            ...
            {selectedDossier && selected && <aside className="country-dossier" aria-label={`Informacje o państwie ${selected.country.name}`}>
              ...
              {(dossierTab === "overview" || dossierTab === "military") && <section className="dossier-stats"><div><span>Sektory</span><b>{selectedDossier.regions.length}</b></div><div><span>Prowincje</span><b>{selectedDossier.provinceCount}</b></div><div><span>Ofensywy</span><b>{selectedDossier.outgoing.length}</b></div><div><span>Obrona</span><b>{selectedDossier.incoming.length}</b></div></section>}
              {(dossierTab === "overview" || dossierTab === "military") && selectedDossier.assessment && <section className={`war-assessment ${selectedDossier.assessment.level}`}><small>TWOJA OFENSYWA PRZECIW TEMU KRAJOWI</small><strong>{selectedDossier.assessment.label}</strong><div><i style={{ width: `${selectedDossier.assessment.chance}%` }} /></div><p>Szansa powodzenia: <b>{selectedDossier.assessment.chance}%</b>. Obrona własnego regionu daje przeciwnikowi +15%; {selectedDossier.assessment.front.attackerFronts > 1 ? `Twoje ${selectedDossier.assessment.front.attackerFronts} fronty rozpraszają siły.` : "nie masz kary za wiele frontów."} Każdy kwartał zawiera jawny czynnik losowy.</p></section>}
              {(dossierTab === "overview" || dossierTab === "military") && selectedDossier.attackableRegions.length > 0 && <section className="dossier-targets"><header><b>DOSTĘPNE CELE</b><span>{selectedDossier.attackableRegions.length}</span></header>{selectedDossier.attackableRegions.map((region) => <button key={region.id} onClick={() => selectStrategicTarget(region.id)}><span>{region.name}</span><b>{formatArea(region.areaKm2)}</b><i>WYBIERZ I POKAŻ →</i></button>)}</section>}
              {(dossierTab === "overview" || dossierTab === "military") && <section className="dossier-regions"><header><b>KONTROLOWANE SEKTORY</b><span>{selectedDossier.regions.length}</span></header><div>{selectedDossier.regions.slice(0, 8).map((region) => <button key={region.id} className={inspectedSectorId === region.id ? "active" : ""} onClick={() => { setInspectedSectorId(region.id); highlightRef.current = engine?.getStrategicRegionIndices(region.id) ?? []; paint(); }}><span>{region.name}</span><b>{region.provinceCount} prow. · {formatArea(region.areaKm2)}</b></button>)}</div>{selectedDossier.regions.length > 8 && <small>oraz {selectedDossier.regions.length - 8} kolejnych sektorów</small>}</section>}
              {(dossierTab === "sector" || dossierTab === "overview") && inspectedSectorId !== null && strategicRegions[inspectedSectorId]?.ownerId === selectedDossier.countryId && <section className="sector-composition"><header><b>SKŁAD SEKTORA</b><span>{strategicRegions[inspectedSectorId].provinceCount}</span></header><strong>{strategicRegions[inspectedSectorId].name}</strong><p>{strategicRegions[inspectedSectorId].provinceNames.join(" · ")}</p></section>}
