...
const capabilityTooltips: Record<keyof typeof componentLabels, string> = {
  economy: "Bieżąca zdolność gospodarki do finansowania państwa i odbudowy. Wpływają na nią baza gospodarcza, wojna, okupacja, migracja oraz inwestycje.",
        ...
        <div className="map-column">
          ...
          <div className="map-frame" ref={mapRef} onContextMenu={(event) => { event.preventDefault(); setSelectedId(null); }}>
            ...
            {selectedDossier && selected && <aside className="country-dossier" aria-label={`Informacje o państwie ${selected.country.name}`}>
              ...
              {dossierTab === "economy" && selectedDossier.strength && <section className="dossier-strength"><div><small title="Łączna, bezwzględna ocena możliwości państwa w skali 0–100. Łączy gospodarkę, ludność, technologię, logistykę, wojsko i instytucje.">POTENCJAŁ</small><strong>{selectedDossier.strength.rating}<i>/100</i></strong><em>{selectedDossier.strength.tier}</em></div><div><small title="Miejsce tego kraju w rankingu wszystkich aktywnych państw według tej samej, bezwzględnej oceny.">RANKING POTENCJAŁU</small><strong>#{selectedDossier.strength.rank || "—"}</strong><em>z {selectedDossier.strength.activeCountries}</em></div><div><small title="Surowy wynik modelu przed zaokrągleniem do widocznej skali 0–100.">WYNIK MODELU</small><strong>{Math.round(selectedDossier.strength.power)}</strong><em>baza 2021 + stan gry</em></div></section>}
              {dossierTab === "economy" && <section className="capacity-breakdown"><header><b>MOŻLIWOŚCI PAŃSTWA</b><span>0–100</span></header><div>{Object.entries(selectedDossier.strength.components).map(([key, value]) => { const component = key as keyof typeof componentLabels; const v = Math.max(0, Math.min(100, value)); return <span key={key} title={capabilityTooltips[component]}><small>{componentLabels[component]}</small><b>{Math.round(v)}</b><i><em style={{ width: `${Math.min(100, v)}%` }} /></i></span>; })}</div><footer><span>Wyczerpanie wojenne <b>{Math.round(selectedDossier.strength.exhaustion)}%</b></span><span>Integracja zdobyczy <b>{Math.round(selectedDossier.strength.integration)}%</b></span></footer></section>}
              {dossierTab === "overview" && selectedDossier.capabilityChanges.length > 0 && <section className="capability-changes"><header><b>ZMIANY POTENCJAŁU</b> <span>ostatnia tura</span></header><div>{selectedDossier.capabilityChanges.map((item) => <span key={item.key} className={`capability-change ${item.trend}`} title={`${capabilityTooltips[item.key as keyof typeof componentLabels]} Wartość duża to mocniejszy zasób, a liczba ze znakiem to zmiana w ostatnim kwartale.`}><small>{item.label}</small><b>{item.value}</b><strong>{item.delta}</strong></span>)}</div><footer>{selectedDossier.countryId === playerCountryId ? <span title="Własne ministerstwa i służby przekazują pełne dane o kraju gracza.">Dane własne <b>pełne</b></span> : <span title="Przybliżony poziom niepewności danych o obcym państwie. Niższa wartość oznacza pewniejszy wywiad.">Niepewność wywiadu <b>{Math.round((engine?.getCountryCapabilityState(selectedDossier.countryId)?.uncertainty ?? 0) * 100)}%</b></span>}<span>Każdy wskaźnik zmienia się w własnym tempie.</span></footer></section>}
              ...
              {dossierTab === "logistics" && <section className="logistics-dossier"><div><small title="Ocena sprawności przemieszczania ludzi, towarów i zaopatrzenia. Liczy dostęp do morza, kolej, drogi, lotniska, rzeki i aktywne inwestycje.">INDEKS LOGISTYCZNY</small><strong>{typeof selectedDossier.countryLogistics === "number" ? Math.round(selectedDossier.countryLogistics) : "—"}</strong><em>/100</em></div><div><span><small title="Słaba logistyka obniża gospodarkę, a bardzo dobra ją wspiera.">WPŁYW NA GOSPODARKĘ</small><b>{selectedDossier.countryLogistics < 30 ? "-0.05/tk" : selectedDossier.countryLogistics > 80 ? "+0.03/tk" : "neutralny"}</b></span><span><small title="Przy indeksie poniżej 40 ogranicza liczbę dostępnych rezerw.">WPŁYW NA REZERWY</small><b>{selectedDossier.countryLogistics < 40 ? "-0.02/tk" : "neutralny"}</b></span></div><details><summary>{(selectedDossier.regionLogistics ?? []).length} regionów</summary><div className="logistics-regions">{(selectedDossier.regionLogistics ?? []).slice(0, 12).map((item) => <span key={item.regionId}><small>{strategicRegions[item.regionId]?.name ?? `Region ${item.regionId}`}</small><b>{Math.round(item.logisticsIndex)}</b><small>port {Math.round(item.maritimeAccess)} · kolej {Math.round(item.railDensity * 100)} · drogi {Math.round(item.roadDensity * 100)} · lotniska {item.airportCount}</small></span>)}</div></details></section>}
        ...
        <aside className="controls">
          ...
          {gameMode === "strategy" ? <div className="strategy-command" aria-live="polite">
            ...
            <div className="strategy-player">
              ...
              {!selectedDossier && typeof playerDossier?.countryLogistics === "number" && <section className="logistics-card"><header><b>LOGISTYKA</b><span>{Math.round(playerDossier.countryLogistics)}/100</span></header><div className="logistics-rows"><span><small>Indeks</small><b>{Math.round(playerDossier.countryLogistics)}</b></span><span><small>Wpływ na gospodarkę</small><b>{playerDossier.countryLogistics < 30 ? "-0.05/tk" : playerDossier.countryLogistics > 80 ? "+0.03/tk" : "neutralny"}</b></span><span><small>Wpływ na manpower</small><b>{playerDossier.countryLogistics < 40 ? "-0.02/tk" : "neutralny"}</b></span></div><details><summary>{(playerDossier?.regionLogistics ?? []).length} regionów</summary><div className="logistics-regions">{(playerDossier?.regionLogistics ?? []).slice(0, 12).map((item) => <span key={item.regionId}><small>{strategicRegions[item.regionId]?.name ?? `Region ${item.regionId}`}</small><b>{Math.round(item.logisticsIndex)}</b><small>port {Math.round(item.maritimeAccess)} · kolej {Math.round(item.railDensity * 100)} · drogi {Math.round(item.roadDensity * 100)} · lotniska {item.airportCount}</small></span>)}</div></details></section>}
