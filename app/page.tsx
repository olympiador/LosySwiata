        ...
        <aside className="controls">
          ...
          {gameMode === "strategy" ? <div className="strategy-command" aria-live="polite">
            ...
            <div className="strategy-player">
              ...
              {typeof playerDossier?.countryLogistics === "number" && <section className="logistics-card"><header><b>LOGISTYKA</b><span>{Math.round(playerDossier.countryLogistics)}/100</span></header><div><span><small>Indeks</small><b>{Math.round(playerDossier.countryLogistics)}</b></span><span><small>Wpływ na gospodarkę</small><b>{playerDossier.countryLogistics < 30 ? "-0.05/tk" : playerDossier.countryLogistics > 80 ? "+0.03/tk" : "neutralny"}</b></span><span><small>Wpływ na manpower</small><b>{playerDossier.countryLogistics < 40 ? "-0.02/tk" : "neutralny"}</b></span></div><details><summary>{(playerDossier?.regionLogistics ?? []).length} regionów</summary><div className="logistics-regions">{(playerDossier?.regionLogistics ?? []).slice(0, 12).map((item) => <span key={item.regionId}><small>{strategicRegions[item.regionId]?.name ?? `Region ${item.regionId}`}</small><b>{Math.round(item.logisticsIndex)}</b><small>port {Math.round(item.maritimeAccess)} · kolej {Math.round(item.railDensity * 100)} · drogi {Math.round(item.roadDensity * 100)} · lotniska {item.airportCount}</small></span>)}</div></details></section>}
              {playerDossier?.regimeType && <section className="regime-card"><header><b>REŻIM</b><span>typ systemu politycznego</span></header><div><span><small>System</small><b>{engine?.getRegimeLabel(playerDossier.regimeType)}</b></span><span><small>Doświadczenie bojowe</small><b>{Math.round(playerDossier.combatExperience)}</b><small>punktów</small></span></div></section>}
