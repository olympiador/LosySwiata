          ...
          <div className="map-frame" ref={mapRef} onContextMenu={(event) => { event.preventDefault(); setSelectedId(null); }}>
            ...
            {selectedDossier && selected && <aside className="country-dossier" aria-label={`Informacje o państwie ${selected.country.name}`}>
              ...
              {dossierTab === "population" && <section className="population-dossier">
                ...
                <details className="regional-population"><summary>Ludność regionów ({selectedDossier.regions.length})</summary><p>Szacunek modelu <InfoTip>Początkowa ludność kraju jest dzielona między regiony proporcjonalnie do ich powierzchni. To nie są rzeczywiste spisy regionalne. Zmiany ludności i struktury wieku kraju są rozdzielane według dotychczasowych udziałów regionów. Podbój przenosi mieszkańców regionu do nowego państwa.</InfoTip></p><div className="logistics-regions">{selectedDossier.regions.map((region) => <button type="button" key={region.id} className={inspectedSectorId === region.id ? "selected" : ""} onClick={() => { setInspectedSectorId(region.id); focusStrategicRegion(region.id); }}><b>{region.name}</b><small>{(engine?.getRegionPopulation(region.id) ?? 0).toLocaleString("pl-PL")} mieszkańców</small></button>)}</div></details>
