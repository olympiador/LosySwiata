...
export default function Home() {
  ...
  const playerDossier = useMemo(() => {
    ...
    return { countryId, regions, provinceCount, strength, outgoing, incoming, occupations, attackableRegions, assessment: null, capabilityChanges, manpower, countryLogistics, regionLogistics, regimeType, informationEnvironment, combatExperience, demographics, demographicType, populationAbsolute, refugeesHosted, borderPolicy, assimilationProgress, playerPolicyState };
        ...
        <div className="map-column">
          ...
          <div className="map-frame" ref={mapRef} onContextMenu={(event) => { event.preventDefault(); setSelectedId(null); }}>
            ...
            {selectedDossier && selected && <aside className="country-dossier" aria-label={`Informacje o państwie ${selected.country.name}`}>
              ...
              {dossierTab === "economy" && selectedDossier.strength && <section className="dossier-strength"><div><small title="Wzorzec łączący gospodarkę, ludność, technologię, logistykę, wojsko i sprawność instytucji. Im wyższy, tym większe możliwości państwa.">POTENCJAŁ</small><strong>{selectedDossier.strength.rating}<i>/100</i></strong><em>{selectedDossier.strength.tier}</em></div><div><small title="Miejsce tego kraju w rankingu wszystkich aktywnych państw według wyniku modelu.">RANKING POTENCJAŁU</small><strong>#{selectedDossier.strength.rank || "—"}</strong><em>z {selectedDossier.strength.activeCountries}</em></div><div><small title="Wartość modelu w oparciu o dane bazowe 2021 roku i aktualny stan gry. Nie to samo co ranking.">WYNIK MODELU</small><strong>{Math.round(selectedDossier.strength.power)}</strong><em>baza 2021 + stan gry</em></div></section>}
              ...
              {dossierTab === "population" && <section className="population-dossier">
                <div><small>LUDNOŚĆ BEZWZGLĘDNA</small><strong>{selectedDossier.populationAbsolute ? Math.round(selectedDossier.populationAbsolute).toLocaleString("pl-PL") + " osób" : "—"}</strong></div>
              ...
              {dossierTab === "military" && <section className="dossier-stats"><div><span>Sektory</span><b>{selectedDossier.regions.length}</b></div><div><span>Prowincje</span><b>{selectedDossier.provinceCount}</b></div><div><span>Ofensywy</span><b>{selectedDossier.outgoing.length}</b></div><div><span>Obrona</span><b>{selectedDossier.incoming.length}</b></div></section>}
              ...
              {dossierTab === "military" && <section className="dossier-regions"><header><b>KONTROLOWANE SEKTORY</b><span>{selectedDossier.regions.length}</span></header><div>{selectedDossier.regions.slice(0, 8).map((region) => <button key={region.id} className={inspectedSectorId === region.id ? "active" : ""} onClick={() => { setInspectedSectorId(region.id); highlightRef.current = engine?.getStrategicRegionIndices(region.id) ?? []; paint(); }}><span>{region.name}</span><b>{region.provinceCount} prow. · {formatArea(region.areaKm2)}</b></button>)}</div>{selectedDossier.regions.length > 8 && <small>oraz {selectedDossier.regions.length - 8} kolejnych sektorów</small>}</section>}
              {dossierTab === "logistics" && <section className="logistics-dossier"><div><small>INDEKS LOGISTYCZNY</small><strong>{typeof selectedDossier.countryLogistics === "number" ? Math.round(selectedDossier.countryLogistics) : "—"}</strong><em>/100</em></div><div><span>Wpływ na gospodarkę<b>{selectedDossier.countryLogistics < 30 ? "-0.05/tk" : selectedDossier.countryLogistics > 80 ? "+0.03/tk" : "neutralny"}</b></span><span>Wpływ na manpower<b>{selectedDossier.countryLogistics < 40 ? "-0.02/tk" : "neutralny"}</b></span></div><details><summary>{(selectedDossier.regionLogistics ?? []).length} regionów</summary><div className="logistics-regions">{(selectedDossier.regionLogistics ?? []).slice(0, 12).map((item) => <span key={item.regionId}><small>{strategicRegions[item.regionId]?.name ?? `Region ${item.regionId}`}</small><b>{Math.round(item.logisticsIndex)}</b><small>port {Math.round(item.maritimeAccess)} · kolej {Math.round(item.railDensity * 100)} · drogi {Math.round(item.roadDensity * 100)} · lotniska {item.airportCount}</small></span>)}</div></details></section>}
              {dossierTab === "policies" && selectedDossier.playerPolicyState && <section className="policy-dossier"><header><b>DECYZJE PREZYDENTA</b><span>punkty decyzyjne: <b className="policy-points">{selectedDossier.playerPolicyState!.decisionPoints}</b></span></header><div className="policy-list">{(engine ? engine.getAvailablePlayerPolicies(selectedDossier.countryId) : []).length ? (engine ? engine.getAvailablePlayerPolicies(selectedDossier.countryId) : []).map((policy) => { const isLogistics = ["build-port", "modernize-roads", "expand-airport", "rail-upgrade"].includes(policy.id); const regionOptions = isLogistics ? (selectedDossier.regions ?? []).filter((r) => r.ownerId === selectedDossier.countryId && (policy.id !== "build-port" || r.maritimeAccess > 0)) : []; return <div key={policy.id} className="policy-item"><div><small>{policy.name}</small><b>{policy.cost} PD · {policy.cooldown} kw. cooldown</b><small>{policy.description}</small></div>{isLogistics && regionOptions.length ? <select disabled={busy || selectedDossier.playerPolicyState!.decisionPoints < policy.cost} onChange={(event) => { if (engine && event.target.value) { engine.activateLogisticsPolicy(policy.id, Number(event.target.value)); refresh(engine); autosave(engine); notify(`Inwestycja: ${policy.name}`); } }}><option value="">Wybierz region...</option>{regionOptions.map((r) => <option key={r.id} value={r.id}>{r.name} · {Math.round(r.areaKm2 ?? 0)} km²</option>)}</select> : <button disabled={busy || selectedDossier.playerPolicyState!.decisionPoints < policy.cost || policy.lastUsedTurn > 0} onClick={() => { if (engine && engine.activatePlayerPolicy(policy.id)) { refresh(engine); autosave(engine); notify(`Aktywowano: ${policy.name}`); } }}>Wykonaj</button>}</div>; }) : <small>Brak dostępnych aktów w tej turze.</small>}</div><footer>{(engine ? engine.getCountryLogisticsInvestments(selectedDossier.countryId) : []).length ? <div><small>Aktywne inwestycje:</small>{(engine ? engine.getCountryLogisticsInvestments(selectedDossier.countryId) : []).slice(0, 4).map((inv) => <span key={inv.id}><b>{inv.type}</b><small>{strategicRegions[inv.regionId]?.name ?? `Region ${inv.regionId}`}</small><small>poz. {inv.remainingTurns} kw.</small></span>)}</div> : <span>Brak aktywnych inwestycji.</span>}</footer></section>}
              {dossierTab === "sector" && inspectedSectorId !== null && (() => { const sector = strategicRegions[inspectedSectorId]; const ownerId = sector?.ownerId ?? selectedDossier.countryId; const owner = engine?.getCountry(ownerId); const regionLogistics = engine?.getRegionLogistics(inspectedSectorId); return <section className="sector-detail"><header><b>{sector?.name ?? `Sektor ${inspectedSectorId}`}</b><span>{owner?.flag} {owner?.name ?? "—"}</span></header><div><span><small>Prowincje</small><b>{sector?.provinceCount ?? 0}</b></span><span><small>Powierzchnia</small><b>{formatArea(sector?.areaKm2 ?? 0)}</b></span><span><small>Logistyka</small><b>{typeof regionLogistics === "number" ? Math.round(regionLogistics) : "—"}</b></span></div><footer>{sector?.provinceNames.length ? <p>{sector.provinceNames.join(" · ")}</p> : <small>Brak danych o prowincjach.</small>}</footer></section>; })()}
            </aside>}
        ...
        <aside className="controls">
          ...
          {gameMode === "strategy" ? <div className="strategy-command" aria-live="polite">
            ...
            <div className="strategy-player">
              ...
              {!(selectedDossier && selectedDossier.countryId === playerCountryId) && playerStrength && <div className="strategy-strength" title={`Potencjał państwa łączy gospodarkę, ludność, technologię, logistykę, wojsko i sprawność instytucji. Zdobycze dają tylko część zasobów do czasu asymilacji, a wyczerpanie wojenne obniża wynik. Siła na konkretnym froncie jest liczona osobno.`}><small>POTENCJAŁ PAŃSTWA</small><strong>{playerStrength.rating}<i>/100</i></strong><em>{playerStrength.tier} · #{playerStrength.rank} z {playerStrength.activeCountries}</em><b>wynik modelu {Math.round(playerStrength.power)} · wyczerpanie {Math.round(playerStrength.exhaustion)}%</b></div>}
              {!(selectedDossier && selectedDossier.countryId === playerCountryId) && playerDossier?.manpower && <div className="manpower-card"><header><b>SIŁY ZBROJNE</b><span>dane wywiadu</span></header><div><span><small>Dysponujący</small><b>{playerDossier.manpower.available}</b><small>na {playerDossier.manpower.active} aktywnych, {playerDossier.manpower.reserves} rezerwy</small></span><span><small>Koszt utrzymania</small><b>{playerDossier.manpower.maintenanceCost.toFixed(1)}</b><small>jednostek/kwartał</small></span><span><small>Mobilizacja</small><b>{playerDossier.manpower.mobilization === "hidden" ? "Ukryta" : playerDossier.manpower.mobilization === "open" ? "Jawna" : "Pełna"}</b><small>{playerDossier.manpower.mobilization === "full" ? "+35% obrony" : playerDossier.manpower.mobilization === "open" ? "+10-25% obrony" : "zwykła gotowość"}</small></span></div></div>}
              {!(selectedDossier && selectedDossier.countryId === playerCountryId) && typeof playerDossier?.countryLogistics === "number" && <section className="logistics-card"><header><b>LOGISTYKA</b><span>{Math.round(playerDossier.countryLogistics)}/100</span></header><div className="logistics-rows"><span><small>Indeks</small><b>{Math.round(playerDossier.countryLogistics)}</b></span><span><small>Wpływ na gospodarkę</small><b>{playerDossier.countryLogistics < 30 ? "-0.05/tk" : playerDossier.countryLogistics > 80 ? "+0.03/tk" : "neutralny"}</b></span><span><small>Wpływ na manpower</small><b>{playerDossier.countryLogistics < 40 ? "-0.02/tk" : "neutralny"}</b></span></div><details><summary>{(playerDossier?.regionLogistics ?? []).length} regionów</summary><div className="logistics-regions">{(playerDossier?.regionLogistics ?? []).slice(0, 12).map((item) => <span key={item.regionId}><small>{strategicRegions[item.regionId]?.name ?? `Region ${item.regionId}`}</small><b>{Math.round(item.logisticsIndex)}</b><small>port {Math.round(item.maritimeAccess)} · kolej {Math.round(item.railDensity * 100)} · drogi {Math.round(item.roadDensity * 100)} · lotniska {item.airportCount}</small></span>)}</div></details></section>}
              {!(selectedDossier && selectedDossier.countryId === playerCountryId) && playerDossier?.regimeType && <section className="regime-card"><header><b>REŻIM</b><span>typ systemu politycznego</span></header><div><span><small>System</small><b>{engine?.getRegimeLabel(playerDossier.regimeType)}</b></span></div></section>}
              {!(selectedDossier && selectedDossier.countryId === playerCountryId) && playerDossier?.combatExperience !== undefined && <section className="experience-card"><header><b>DOŚWIADCZENIE BOJOWE</b><span>punkty doświadczenia</span></header><div><span><small>Suma</small><b>{Math.round(playerDossier.combatExperience)}</b><small>punktów</small></span></div></section>}
              {!(selectedDossier && selectedDossier.countryId === playerCountryId) && playerDossier?.informationEnvironment && <section className="information-card"><header><b>ŚRODOWISKO INFORMACYJNE</b><span>{engine?.getInformationEnvironmentLabel(playerDossier.informationEnvironment.score)}</span></header><div><span><small>Technologia</small><b>{Math.round(playerDossier.informationEnvironment.techComponent)}</b><small>dostęp do platform</small></span><span><small>Kontrola mediów</small><b>{Math.round(playerDossier.informationEnvironment.mediaControl)}</b><small>cenzura/propaganda</small></span><span><small>Służby specjalne</small><b>{Math.round(playerDossier.informationEnvironment.servicesStrength)}</b><small>inwigilacja</small></span></div></section>}
              {!(selectedDossier && selectedDossier.countryId === playerCountryId) && playerDossier?.demographics && <section className="demographics-card"><header><b>DEMOGRAFIA</b><span>{engine?.getDemographicLabel(playerDossier.demographicType)}</span></header><div className="demographics-bars">{Object.entries(playerDossier.demographics).map(([key, value]) => <span key={key}><small>{key === "children" ? "Dzieci 0-14" : key === "youth" ? "Młodzież 15-24" : key === "primeAge" ? "Najlepszy wiek 25-44" : key === "middleAge" ? "Średni wiek 45-64" : key === "elderly" ? "Seniorzy 65+" : "80+ lat"}</small><b>{Math.round((value ?? 0) * 100)}%</b><i><em style={{ width: `${Math.min(100, Math.max(0, (value ?? 0) * 100))}%` }} /></i></span>)}</div><footer><span>Ludność bezwzględna: <b>{engine?.getCountryPopulationAbsolute(playerDossier.countryId) ? Math.round(engine.getCountryPopulationAbsolute(playerDossier.countryId)).toLocaleString("pl-PL") + " osób" : "—"}</b></span><span>Typ piramidy: <b>{engine?.getDemographicLabel(playerDossier.demographicType)}</b></span><span>Granice: <b>{playerDossier.borderPolicy === "closed" ? "Zamknięte" : playerDossier.borderPolicy === "selective" ? "Selektywne" : playerDossier.borderPolicy === "open" ? "Otwarte" : "Masowe"}</b></span>{playerDossier.refugeesHosted > 0 && <span>Uchodźcy: <b>{Math.round(playerDossier.refugeesHosted).toLocaleString("pl-PL")}</b></span>}</footer></section>}
              {!(selectedDossier && selectedDossier.countryId === playerCountryId) && playerDossier?.playerPolicyState && (() => {
                ...
                return <section className="policy-card"><header><b>DECYZJE PREZYDENTA</b><span>punkty decyzyjne: <b className="policy-points">{policyState.decisionPoints}</b></span></header><div className="policy-list">{policies.length ? policies.map((policy) => {
                  const isLogistics = ["build-port", "modernize-roads", "expand-airport", "rail-upgrade"].includes(policy.id);
                  const regionOptions = isLogistics ? countryRegions.filter((r) => policy.id === "build-port" ? r.maritimeAccess > 0 : true) : [];
                  return <div key={policy.id} className="policy-item"><div><small>{policy.name}</small><b>{policy.cost} PD · {policy.cooldown} kw. cooldown</b><small>{policy.description}</small></div>{isLogistics && regionOptions.length ? <select disabled={busy || policyState.decisionPoints < policy.cost} onChange={(event) => { if (engine && event.target.value) { engine.activateLogisticsPolicy(policy.id, Number(event.target.value)); refresh(engine); autosave(engine); notify(`Inwestycja: ${policy.name}`); } }}><option value="">Wybierz region...</option>{regionOptions.map((r) => <option key={r.id} value={r.id}>{r.name} · {Math.round(r.areaKm2 ?? 0)} km²</option>)}</select> : <button disabled={busy || policyState.decisionPoints < policy.cost || policy.lastUsedTurn > 0} onClick={() => { if (engine && engine.activatePlayerPolicy(policy.id)) { refresh(engine); autosave(engine); notify(`Aktywowano: ${policy.name}`); } }}>Wykonaj</button>}</div>;
            ...
            {playerOccupations.length > 0 && <section className="occupation-list"><header><b>ASYMILACJA ZDOBYCZY</b><span>{playerOccupations.length} równocześnie · wolniej</span></header>{playerOccupations.slice(0, 5).map((occupation) => <div key={occupation.regionId}><span>{strategicRegions[occupation.regionId]?.name}</span><b>{Math.round(occupation.progress)}%</b><i><em style={{ width: `${occupation.progress}%` }} /></i><small>{occupation.progress < 30 ? <span style={{ color: "var(--red)" }}>Kary: -2 gospodarka, -2 logistyka, -2 stabilność, -1 manpower</span> : <span style={{ color: "var(--teal)" }}>Zintegrowano</span>}</small></div>)}</section>}
