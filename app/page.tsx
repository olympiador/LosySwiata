        ...
        <aside className="controls">
          ...
          {gameMode === "strategy" ? <div className="strategy-command" aria-live="polite">
            <div className="strategy-player">
              ...
              {typeof selectedDossier?.countryLogistics === "number" && <section className="logistics-card"><header><b>LOGISTYKA</b><span>{Math.round(selectedDossier.countryLogistics)}/100</span></header><div><span><small>Indeks</small><b>{Math.round(selectedDossier.countryLogistics)}</b></span><span><small>Wpływ na gospodarkę</small><b>{selectedDossier.countryLogistics < 30 ? "-0.05/tk" : selectedDossier.countryLogistics > 80 ? "+0.03/tk" : "neutralny"}</b></span><span><small>Wpływ na manpower</small><b>{selectedDossier.countryLogistics < 40 ? "-0.02/tk" : "neutralny"}</b></span></div><footer>{(selectedDossier?.regionLogistics ?? []).slice(0, 5).map((item) => <span key={item.regionId}><small>{strategicRegions[item.regionId]?.name ?? `Region ${item.regionId}`}</small><b>{Math.round(item.logisticsIndex)}</b><small>port {Math.round(item.maritimeAccess)} · kolej {Math.round(item.railDensity * 100)} · drogi {Math.round(item.roadDensity * 100)} · lotniska {item.airportCount}</small></span>)}</footer></section>}
              ...
              {selectedDossier?.playerPolicyState && selectedDossier.countryId === playerCountryId && (() => {
                ...
                const countryRegions = (selectedDossier.regions ?? []).filter((r) => r.ownerId === playerCountryId);
                ...
                }) : <small>Brak dostępnych aktów w tej turze.</small>}</div><footer>{engine?.getCountryLogisticsFromRegions(playerCountryId) !== undefined && <span>Aktywne inwestycje logistyczne: <b>{engine.getCountryLogisticsFromRegions(playerCountryId) ?? 0}</b></span>}</footer></section>;
              })()}
            </div>
